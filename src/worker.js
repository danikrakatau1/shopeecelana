...
const verifySessionToken = async (token, secret) => {
  ...
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    if (!payload?.exp || payload.exp <= now || payload.sub !== 'owner') return null;
    return payload;
  } catch (_) {
    return null;
  }
...
const clearShopeeCookie = () =>
  `${SHOPEE_COOKIE}=; Path=/api/shopee; HttpOnly; Secure; SameSite=Lax; Max-Age=0`;

const clearOauthCookie = () =>
  `${OAUTH_COOKIE}=; Path=/shopee/callback; HttpOnly; Secure; SameSite=Lax; Max-Age=0`;

const deriveAesKey = async (secret) => {
  const digest = await crypto.subtle.digest(
    'SHA-256',
    encoder.encode(`ARSTORE_SHOPEE_TOKEN_V1:${secret}`)
  );
...
const addPrivateHeaders = (response) => {
  ...
  headers.set(
    ...
    "default-src 'self'; script-src 'self'; style-src 'self' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self'; img-src 'self' data:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
  );
...
const handleLogin = async (request, env) => {
  ...
  if (!safeEqual(username, expectedUsername) || !safeEqual(password, env.SELLER_PASSWORD)) {
    return json({ error: 'invalid_credentials' }, 401);
  }

  const token = await createSessionToken(env.SESSION_SECRET, 'owner');
  return json(
    { ok: true, user: { role: 'owner', username: expectedUsername }, expiresIn: SESSION_TTL },
    200,
...
const handleSession = async (request, env) => {
  ...
  return json({
    authenticated: true,
    user: { role: 'owner', username: env.SELLER_USERNAME || 'owner' },
...
const handleShopeeStatus = async (request, env) => {
  ...
  const connected = Boolean(configured && token?.access_token && token?.shop_id);
  ...
  return json({
    ...
    callbackUrl: env.SHOPEE_REDIRECT_URL || `${new URL(request.url).origin}/shopee/callback`,
    storage: connected ? 'encrypted HttpOnly cookie' : 'not connected'
  });
};
...
const handleShopeeConnect = async (request, env) => {
  const auth = await requireSession(request, env);
  if (!auth.ok) return auth.response;
  if (!shopeeCredentialsReady(env)) {
    return json({ error: 'shopee_not_configured', missing: ['SHOPEE_PARTNER_ID', 'SHOPEE_PARTNER_KEY'] }, 503);
  }
  ...
  const redirectUrl = env.SHOPEE_REDIRECT_URL || `${requestUrl.origin}/shopee/callback`;
  const authUrl = new URL(`${getShopeeBase(env)}${path}`);
  ...
  headers.append(
    'set-cookie',
    `${OAUTH_COOKIE}=${oauthNonce}; Path=/shopee/callback; HttpOnly; Secure; SameSite=Lax; Max-Age=600`
  );
...
const handleShopeeCallback = async (request, env) => {
  ...
  if (!shopeeCredentialsReady(env) || !env.SESSION_SECRET) {
    return callbackFailure(requestUrl.origin, 'configuration');
  }

  const cookies = parseCookies(request);
  if (!cookies[OAUTH_COOKIE]) return callbackFailure(requestUrl.origin, 'oauth_session');
  ...
  try {
    ...
    return callbackFailure(requestUrl.origin, 'token_exchange');
  }

  if (!tokenResponse?.access_token || tokenResponse?.error) {
    return callbackFailure(requestUrl.origin, 'token_rejected');
  }
  ...
  headers.append(
    'set-cookie',
    `${SHOPEE_COOKIE}=${encrypted}; Path=/api/shopee; HttpOnly; Secure; SameSite=Lax; Max-Age=${SHOPEE_COOKIE_TTL}`
  );
...
const handleShopInfo = async (request, env) => {
  const auth = await requireSession(request, env);
  if (!auth.ok) return auth.response;
  if (!shopeeCredentialsReady(env)) return json({ error: 'shopee_not_configured' }, 503);

  const token = await getShopeeToken(request, env);
  if (!token?.access_token || !token?.shop_id) return json({ error: 'shopee_not_connected' }, 401);

  const now = Math.floor(Date.now() / 1000);
  if (token.expires_at && token.expires_at <= now) return json({ error: 'shopee_token_expired' }, 401);
  ...
  endpoint.searchParams.set('shop_id', token.shop_id);
  endpoint.searchParams.set('sign', sign);
  ...
  try {
    ...
    if (!response.ok || data?.error) {
      return json({ error: 'shopee_api_error', message: data?.message || data?.error || 'Request failed' }, 502);
    }
    return json({ ok: true, shop: data?.response || data });
  } catch (_) {
    return json({ error: 'shopee_api_unreachable' }, 502);
  }
};
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/dashboard' || url.pathname.startsWith('/dashboard/')) {
      if (!env.SELLER_PASSWORD || !env.SESSION_SECRET) {
        return new Response('Seller authentication is not configured yet.', {
          status: 503,
    ...
    if (url.pathname === '/seller-login' || url.pathname.startsWith('/seller-login/')) {
      return servePrivateAsset(request, env);
    }
